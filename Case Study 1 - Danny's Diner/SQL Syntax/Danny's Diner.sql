...
SELECT TOP 3 customer_id, product_name, MAX(aaa) as most_popular
FROM (
    SELECT s.customer_id, m.product_name, SUM(s.product_id) AS aaa
    FROM sales AS s
...
-- total items -> unique menu items or total items purchased?
-- Total Items purchased prior to Membership
SELECT s.customer_id, COUNT(s.product_id) AS total_purchases, SUM(m.price) AS total_amount_spent
  FROM sales AS s
  INNER JOIN members AS ms ON s.customer_id = ms.customer_id
  INNER JOIN menu AS m ON s.product_id = m.product_id
  WHERE s.order_date < ms.join_date
GROUP BY s.customer_id;
-- Unique menu items purchased prior to Membership
SELECT s.customer_id,  COUNT(DISTINCT s.product_id) AS unique_item_purchase, SUM(price) AS total_amount_spent--, (price * SUM(product_id)) AS total_spent
FROM sales AS s
...
SELECT d.customer_id,
	...
	END) AS points
FROM DATES_CTE AS d
  INNER JOIN sales AS s ON d.customer_id = s.customer_id
  INNER JOIN menu AS m ON s.product_id = m.product_id
